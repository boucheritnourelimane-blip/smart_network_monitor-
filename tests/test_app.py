...
def test_login_post_valid(client):
    response = client.post('/', data={'username': 'admin', 'password': '1234'}, follow_redirects=True)
    assert response.status_code == 200
...
def test_add_device_authenticated(client):
    client.post('/', data={'username': 'admin', 'password': '1234'})
    response = client.post('/add', data={'name': 'Test', 'ip': '1.1.1.1', 'port': '80', 'type': 'Routeur'}, follow_redirects=True)
    assert response.status_code == 200

def test_add_device_without_login(client):
    response = client.post('/add', data={'name': 'Test'}, follow_redirects=True)
    assert response.status_code == 200
